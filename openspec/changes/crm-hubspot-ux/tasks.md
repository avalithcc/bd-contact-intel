...
- [x] 3.4 `--execute --run=<id>`: refuses if `input_hash` changed, run not approved, or run already executed; snapshots production first (`pg_dump` + `pg_restore --list` verification, owner-approved mechanism); tags rows with `migration_run_id`. Execution bookkeeping links back to the SAME approved `migration_run` row (sets `mode='execute'`, `executed_at`) rather than orphaning a new one, and writes one `audit_log(migration_execute)` entry in the same transaction as the writes.
