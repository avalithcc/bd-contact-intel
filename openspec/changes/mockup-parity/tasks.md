...
- [ ] 6.2 `/tasks`, `/companies`, company record restyle + modal migration.
  ...
  - REMAINING (not started this batch): `/companies/[key]` company record (~140-line CSS + 114-line page) and `/companies/new` (~107-line page), plus `EditCompanyModal`/`AddActivityModal` → shared `Dialog` migration — targeted for `06c`. `/hiring`, `/discovery`, `/whats-new` (6.1) also remain, targeted for `06d`.
