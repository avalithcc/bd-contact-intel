...
import { JOB_SOURCES, type AtsKey } from "./registry";
...
// Reuses registry.ts's AtsKey rather than hand-maintaining a separate union
// here — see the PROBE_ENTRIES comment below for why: this alias existing
// as its own hardcoded list (independent of JOB_SOURCES) is exactly how
// discovery ended up probing recruitee/teamtailor with no sync adapter for
// either.
export type ProbeAts = AtsKey;
...
/**
 * Wraps a JobSource adapter (see registry.ts) as a discovery probe: reuses
 * its fetch + normalize logic as-is rather than duplicating it, and treats
 * ANY thrown error (404 for a wrong slug, timeout, shape drift) as a miss
 * rather than letting it propagate — a wrong slug guess is the expected,
 * common case here, not a failure worth recording as one. Also treats an
 * empty result as a miss, which is what makes SmartRecruiters' "HTTP 200
 * with `{ totalFound: 0, content: [] }` for any slug" behavior harmless
 * here: an empty array's `.length` is falsy regardless of the 200 status,
 * so it never becomes a hit.
 */
...
/**
 * Probe list, derived directly from `JOB_SOURCES` (registry.ts) instead of
 * a hand-maintained list here. This is the actual fix for the gap this
 * change closes: discovery used to probe recruitee/teamtailor via bespoke,
 * duplicated fetch logic while registry.ts had no adapter for either, so
 * approving one of those boards produced a permanent `No adapter
 * registered for ats "..."` sync_run error — the two lists had drifted
 * apart and nothing caught it. Deriving from `JOB_SOURCES` makes that class
 * of drift structurally impossible: every ATS discovery probes now IS one
 * `syncCompany` (sync.ts) already knows how to fetch, by construction. A
 * future adapter added to `JOB_SOURCES` starts getting probed automatically
 * with no change needed here at all.
 */
const PROBE_ENTRIES: [AtsKey, ProbeFn][] = Object.entries(JOB_SOURCES).map(
  ([ats, source]) => [ats as AtsKey, adapterProbe(source as JobSource)],
);
 ...
 * registered ATS (PROBE_ENTRIES, currently 6) within a round run
 * concurrently (they're different hosts, so this is a latency win, not a
 * politeness problem — see SLUG_ROUND_DELAY_MS for the actual politeness
 * delay, applied between rounds). Stops at the first round that produces
 * any hit — once a company's board is found, further slug guesses for it
 * add cost without adding value. Every hit found in the winning round is
 * recorded (a company can legitimately have boards on more than one ATS,
 * though that's rare).
 *
 * Never throws: every probe already swallows its own errors (see
 * adapterProbe above), so a bad guess is just a miss.
 */
async function probeCompany(company: CandidateCompany): Promise<boolean> {
  ...
  for (let i = 0; i < slugs.length; i++) {
    ...
    const results = await Promise.all(
      PROBE_ENTRIES.map(async ([ats, probe]) => ({ ats, hit: await probe(slug) })),
    );
